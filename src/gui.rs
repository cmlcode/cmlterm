...
use eframe::egui::{ self, CentralPanel, Color32, Event, InputState, Key, Rect, TextStyle, Ui, FontData, FontFamily, FontDefinitions };
...
fn setup_fonts(ctx: &egui::Context) {
    ...
    fonts.font_data.insert(
        ...
        FontData::from_static(include_bytes!("../res/HackNerdFontMono-Regular.ttf")).into(),
    );

    fonts.font_data.insert(
        BOLD_FONT_NAME.to_owned(),
        FontData::from_static(include_bytes!("../res/HackNerdFontMono-Bold.ttf")).into(),
    );
...
impl eframe::App for TerminalGui {
...
pub fn run(terminal_emulator: TerminalEmulator) {
    ...
    eframe::run_native(
        "cmlterm",
        native_options,
        Box::new(move |cc| Ok(Box::new(TerminalGui::new(cc, terminal_emulator)))),
    )
    .unwrap();
}
